...
export default function SchoolFilters({ filters, onChange, onReset }) {
    ...
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <input type="number" min="0" max={MAX_SCORE} className="input" placeholder={t('filters_min_score')} value={filters.minScore} onChange={(e) => update('minScore', e.target.value)} />
          <input type="number" min="0" max={MAX_SCORE} className="input" placeholder={t('filters_max_score')} value={filters.maxScore} onChange={(e) => update('maxScore', e.target.value)} />
        </div>
      </div>
      ...
      <div>
        ...
        <select className="input" value={filters.studyDuration} onChange={(e) => update('studyDuration', e.target.value)}>
          <option value="">{t('filters_all')}</option>
      ...
      <button className="btn-outline flex w-full items-center justify-center gap-2 hover:border-red-300 hover:bg-red-50 hover:text-red-600" onClick={onReset}>
  ...
  return (
    <>
      <button onClick={() => setMobileOpen(true)} className="btn-outline relative mb-4 inline-flex w-full items-center justify-center gap-2 lg:hidden">
        <FiFilter className="h-4 w-4" />
        {t('filters_title')}
        {activeCount > 0 && <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-teal-500 px-1.5 text-[11px] font-semibold text-white">{activeCount}</span>}
      </button>
